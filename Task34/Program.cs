...
public class Answer {
  public static double[, ] CreateRandomMatrix(int m, int n, int minLimitRandom, int maxLimitRandom) {
    ...
    Console.WriteLine("Введите минимальную и максимальную границу случайных чисел");
    
    int inLimitRandom = Convert.ToInt32(Console.ReadLine());
...
{
array[i, j] = new Random().NextDouble(inLimitRandom, maxLimitRandom);
